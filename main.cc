...
double teta_i = Pi/4 + delta_teta_i; //Pi/2; // 
double fii_i = Pi/2;
//
// ierosmes noskanjoshana	//SVARIGI!!!!!!!
int Fg_d = 4;// 4; //3;//4; //1; //2; //1; //
int Fe_d = 3; //3; //3; //3;//1; //1; //3; //1; //
//
//
// zondes noskanjoshana	//SVARIGI!!!!!!!
int Fg_p = 4; //4; //3;//2; //1; //
int Fe_p = 4; //4;//4; //4; //1; //3; //1; //
//
//noveerosanas geometrija	//SVARIGI!!!!!!!
double delta_teta = 0.0 * Pi/180;
double delta_fii = 0.0 * Pi/180;
//
int pol_o[2] = {1, -1};  //{0, 0}; //0 - lineaara; 1 - pa labi; -1 - pa kreisi //pol_o[2] nozime ka output bus 2 komponetnes
double teta_o[2] = {Pi/2 + delta_teta, Pi/2 + delta_teta}; //ar komatu atdala pirmo un otro komponenti
double fii_o[2] = {0 + delta_fii, 0 + delta_fii}; //{0, Pi/2}; //
//
//zondējošā stara ģeometrija	//SVARIGI!!!!!!!
//
int pol_p[2] = {1, -1};  //{0, 0}; //0 - lineaara; 1 - pa labi; -1 - pa kreisi //pol_o[2] nozime ka output bus 2 komponetnes
double teta_p[2] = {Pi/2 + delta_teta, Pi/2 + delta_teta}; //{0, Pi/2}; // ar komatu atdala pirmo un otro komponenti
double fii_p[2] = {0 + delta_fii, 0 + delta_fii}; //{0, Pi/2}; //{0, Pi/2}; //{Pi, Pi}; //{Pi, Pi}; //
...
int main(){
	...
	lShift_probe = 24.9; // crossover(3;5) nprobe_det;//0; //-5316; //-1919; // 0; // -2280; //0; // //Izskanosana no parejas, vienibas - MHz
	...
	for(int Ri = 3; Ri <= 7; Ri += 4){ //sheit izvelas un ievieto vajdzigos rabi indeksus, kas atbilsti rabi frekvencem kas definetas augstak/ Taisa rabi ciklus sakas ar Ri= un beigu vertiba Ri< un Ri+= saka par cik palielinat katru nakamo
